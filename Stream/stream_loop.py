...
async def update_streams():
    ...
    """
    while True:
        r = requests.get(config["stream"]["twitch_api_url"],
                         params={'game': config["stream"]["twitch_game"], "client_id": tokens["twitch_client_id"]},
        ...
        try:
            ...
            logger.info("Couldn't load twitch streams, but response is json: {}".format(r.text))
        else:
            if len(streams) == 0:
